            ...
            <table id="books-table" class="table table-striped table-bordered" width="100%" cellspacing="0">
                ...
                <tbody>
                    @foreach ($categor as $category)
                        <tr>
                            <td>
                                <a href="{{ route('publishers.show', $category) }}">{{ $category->name }}</a>
                            </td>
                            <td>{{ $category->addreas }}</td>
                            <td class="d-flex justify-content-start align-items-center">
                                <form action="{{ route('publishers.destroy', $category->id) }}" method="post" class="me-2">
                                    @csrf
                                    @method('delete')
                                    <button class="btn btn-danger btn-sm" type="submit">حذف</button>
                                </form>
                                <a class="btn btn-info btn-sm" href="{{ route('publishers.edit', $category) }}">
